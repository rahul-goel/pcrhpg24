...
__device__ void rasterize(const ChangingRenderData& data, unsigned long long int* framebuffer, float3 point, unsigned int index, int &count)
{
	float4 pos = matMul(data.uTransform, make_float4(point, 1.0f));
  ...
  __syncthreads();
    ...
    if (i == 8) {
      ...
      if (s_count == 0) return;
		...
		// rasterize(data, framebuffer, point, index);
		rasterize(data, framebuffer, point, index, count);
	}
#endif
